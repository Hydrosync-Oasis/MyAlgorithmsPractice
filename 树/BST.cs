namespace 树 {
    public class BST<T> where T : IComparable {
        ...
        public void Remove(T element) {
            ...
            if (element.Equals(root.val)) {//判断是否删除根节点
        ...
        private void RecursionRemove(T element, Node currentNode) {
            try {
                ...
                if (wannaDelete is not null) {//找到目标值并已存储了该目标值的父亲节点
                    if (isLeft)//开始删除节点
                    {
        ...
        public int GetCount() => RecursionGetCount(root);
        ...
        public T[] IteratePreOrder3() {
            ...
            while (stck.Count != 0) {
                ...
                if (!used.Contains(thsNode)) {//首次遍历到一个节点先把节点的左右孩子全部加入栈中
        ...
        public static explicit operator T[](BST<T> binaryTree) => binaryTree.IterateSeqOrder();
    }
}
