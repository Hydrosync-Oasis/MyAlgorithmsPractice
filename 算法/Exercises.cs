    ...
    internal static class Exercises
    {
        ...
        */
        ...
        public static int[] GetPrefixArr(string s)
        {
            ...
            for (int i = 1; i < s.Length; i++)
            {
                if (s[i] != s[j])//不匹配了
                {
                    ...
                    while (j >= 1 && s[j] != s[i])
                    {
                        j = next[j - 1];//回退原理=查找字符串原理
                    }
                    //循环结束，找到了应回退到的位置了，此位置反应了最长相等前后缀的长度 - 1

                }

                if (s[i] == s[j])//不满足 j一定 = 0
                {
                    j++;//两个作用：1. 让j指针向后移。2. 让索引加一成为长度
                }
        ...
        public static int MaxProductDP(int[] nums)
        {
            ...
            for (int i = 1; i < nums.Length; i++)
            {
                dp1[i] = Math.Max(Math.Max(nums[i], dp1[i - 1] * nums[i]), dp2[i - 1] * nums[i]);//负号变单调性，最小也要变最大
                dp2[i] = Math.Min(Math.Min(nums[i], dp1[i - 1] * nums[i]), dp2[i - 1] * nums[i]);//负号变单调性，最大也要变最小
